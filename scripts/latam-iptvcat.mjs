name: LATAM iptvcat (single-scrape)
  ...
  iptvcat:
    ...
    defaults:
      run:
        shell: bash
    env:
      # Start URL (category/listing). Example keeps Latin America page:
      IPTVCAT_START_URL: "https://iptvcat.com/latin_america__7/"
      # Crawl depth: 0 = only the start page; 1..N = allow more pages if discovered
      IPTVCAT_MAX_PAGES: "0"
      # Optional: override with a single "my_list" .m3u8 URL if you want a one-shot scrape
      IPTVCAT_MYLIST_URL: ""
      # Network
    ...
    steps:
      ...
      - name: Install deps (no Playwright)
        run: |
          set -euo pipefail
          npm i --no-save @supabase/supabase-js luxon cheerio

      - name: Write iptvcat scraper (fetch-only)
        run: |
          set -euo pipefail
          ...
          async function collectPages(startUrl) {
            ...
            try {
              ...
              // Discover simple pagination links that remain under same section
              ...
              $('a[href]').each((_, a) => {
                ...
                try {
                  ...
                  // limit to same section to avoid going wide; tweak as needed
                  if (u.startsWith(startUrl) || /latin_america__7\/\d+\/?$/.test(u)) set.add(u);
                } catch {}
              });
              const urls = [...set];
          ...
          function parseListingRows(html, baseUrl) {
            const $ = cheerio.load(html);
            const out = [];
            // Heuristic: for each TR, grab channel name (first td?), download link (/my_list), direct .m3u8s, country guess
            $('tr').each((_, tr) => {
              const $tr = $(tr);
              const tds = $tr.find('td');
              if (!tds.length) return;
              const name = textOf($(tds.get(0)));
              const country = textOf($(tds.last()));
              const listLink = $tr.find('a[href*="/my_list/"]').first();
              const download_url = listLink.length ? new URL(listLink.attr('href'), baseUrl).href : null;

              const direct = [];
              $tr.find('a[href$=".m3u8"]').each((_, a) => {
                try { direct.push(new URL($(a).attr('href'), baseUrl).href); } catch {}
              });
          ...
          async function probePlaylist(url) {
            ...
            try {
              const r = await fetch(url, {
                headers: { 'user-agent': UA, 'accept': 'application/vnd.apple.mpegurl,text/plain,*/*' },
                redirect: 'follow',
                signal: ac.signal
              });
          ...
          async function main() {
            ...
            if (MYLIST_URL) {
              ...
              const targets = [...new Set(rawUrls)].map(u => ({ channel_name: null, country_hint: null, candidate_url: u }));
              ...
              const ups = probed.filter(p => p.ok).map(p => ({
                stream_url: p.url,
                channel_name: p.channel_name,   // may be null if the list lacks names
            ...
            const candidates = [];
            for (const E of expanded) for (const u of E.m3u8s) {
              candidates.push({ name: E.channel_name, country: E.country_hint, page: E.source_page, url: u });
            }
            await fs.writeFile(path.join('out','latam','probe_targets.json'), JSON.stringify({ count: candidates.length }, null, 2), 'utf8');
      ...
      - name: Upload artifacts
        ...
        with:
          ...
          path: |
            ...
            out/latam/lists_expanded.json
            out/latam/probe_targets.json
            out/latam/probes.json
            out/latam/best.json
            out/latam/streams_latam.json
            out/latam/mylist_body.m3u8
