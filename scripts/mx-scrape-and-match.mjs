...
function normalizeNumerals(s){const m={uno:'1',dos:'2',tres:'3',cuatro:'4',cinco:'5',seis:'6',siete:'7',ocho:'8',nueve:'9',diez:'10',once:'11',doce:'12',trece:'13'};return String(s).replace(/\b(uno|dos|tres|cuatro|cinco|seis|siete|ocho|nueve|diez|once|doce|trece)\b/gi,x=>m[x.toLowerCase()]);}
...
function tokensOf(s){if(!s)return[];let p=stripAccents(normalizeNumerals(String(s).toLowerCase()));p=dropTimeshift(p);p=stripCountryTag(p);p=p.replace(/&/g,' and ').replace(/[^a-z0-9]+/g,' ').trim();return p.split(/\s+/).filter(t=>t&&!STOP.has(t));}
function keyOf(s){return Array.from(new Set(tokensOf(s))).sort().join(' ');}
function expandNameVariants(s){if(!s)return[];const out=new Set();const orig=String(s).trim();const noCanal=stripLeadingCanal(orig);const flat=x=>x.replace(/[._]+/g,' ').replace(/\s+/g,' ').trim();const noTS=dropTimeshift(noCanal);const noCountry=stripCountryTag(noTS);[orig,noCanal,noTS,noCountry,flat(orig),flat(noCanal),flat(noTS),flat(noCountry)].forEach(v=>{if(v)out.add(v);});return[...out];}
...
async function collectChannelPages(browser){
  ...
  let items=await page.$$eval('a[href*="/channels/"]',as=>{
    const out=[];for(const a of as){const href=a.getAttribute('href')||'';if(!href.includes('/channels/'))continue;const url=new URL(href,location.href).href;const name=(a.textContent||'').trim();out.push({url,name});}
    const m=new Map();for(const it of out)if(!m.has(it.url))m.set(it.url,it);return[...m.values()];
  });
  items=items.filter(i=>i.name&&i.url);items=uniqBy(items,x=>x.url);
...
async function scrapeChannel(browser,link){
  const page=await browser.newPage();page.setDefaultTimeout(NAV_TIMEOUT_MS);
  try{
    await page.goto(link.url,{waitUntil:'domcontentloaded'});await page.waitForTimeout(500);
    ...
    if(!anchors.length){const html=await page.content();const rx=/https?:\/\/[^\s"'<>]+\.m3u8[^\s"'<>]*/gi;const set=new Set();let m;while((m=rx.exec(html)))set.add(m[0]);anchors=[...set].map(u=>({url:u,text:''}));}
  ...
  }catch(e){console.error(`Error scraping ${link.url}: ${e.message}`);return [];}finally{await page.close();}
}
async function scrapeAll(browser,links){
  const out=[];for(const lnk of links){const streams=await scrapeChannel(browser,lnk);if(streams.length){out.push({channelName:lnk.name,channelNameKey:lnk.nameKey,streams});}await delay(PER_PAGE_DELAY_MS);}return out;
}
async function probeM3U8(url){
  const ac=new AbortController();const t=setTimeout(()=>ac.abort(),PROBE_TIMEOUT_MS);
  try{const r=await fetch(url,{method:'GET',headers:{'user-agent':'Mozilla/5.0','accept':'application/vnd.apple.mpegurl,text/plain,*/*'},signal:ac.signal});if(!r.ok)return false;const txt=await r.text();return txt.includes('#EXTM3U');}
  catch{return false;}finally{clearTimeout(t);}
}

// ---------- STREAM-PARSE EPG SAFELY ----------
function isMexicoEntry(id,names){return /\.mx$/i.test(id)||names.some(n=>/\b(m[eé]xico|mx)\b/i.test(String(n)));}
async function parseEpgStream(){
  console.log(`Downloading EPG (stream)… ${EPG_GZ_URL}`);
  const res=await fetch(EPG_GZ_URL); if(!res.ok||!res.body) throw new Error(`Fetch failed ${res.status} ${EPG_GZ_URL}`);
  const gunzip=createGunzip(); const src=Readable.fromWeb(res.body); const decoder=new TextDecoder('utf-8'); const parser=new SaxesParser({xmlns:false});

  const idTo=new Map(); const nameMap=new Map(); const programmesSeen=new Set();
  ...
  parser.on('opentag',(tag)=>{
    ...
    else if(nm==='programme'){const cid=tag.attributes?.channel; if(cid) programmesSeen.add(String(cid));}
  });
  parser.on('text',(t)=>{
  ...
  parser.on('closetag',(raw)=>{
    ...
    if(nm==='display-name'&&cur){
      ...
      if(isMexicoEntry(id,cur.namesRaw)){
        ...
        const entry={id,names:limited,tokenSet:new Set(),hasProgs:false};
        for(const nm2 of entry.names) for(const tok of tokensOf(nm2)) entry.tokenSet.add(tok);
        idTo.set(id,entry);
        for(const n of entry.names){const k=keyOf(n); if(k&&!nameMap.has(k)) nameMap.set(k,entry);}
      }
  ...
  await new Promise((resolve,reject)=>{
    src.on('error',reject); gunzip.on('error',reject);
  ...
  for(const cid of programmesSeen){const o=idTo.get(cid); if(o) o.hasProgs=true;}
  for(const [k,v] of nameMap.entries()) if(!v.hasProgs) nameMap.delete(k);

  console.log(`EPG entries kept (Mexico-related): ${new Set([...nameMap.values()]).size}`);
  return {nameMap, entries:[...new Set([...nameMap.values()])]};
}

// ---------- MATCH ----------
function jaccard(a,b){const A=new Set(a),B=new Set(b); let inter=0; for(const t of A) if(B.has(t)) inter++; return inter/(A.size+B.size-inter||1);}
function findMatch(channelName,nameKey,nameMap,entries){
  ...
  for(const e of entries){const E=e.tokenSet||new Set(); let ok=true; for(const t of sTok){if(!E.has(t)){ok=false;break;}} if(ok&&E.size<subsetBestSize){subsetBest=e;subsetBestSize=E.size;}}
...
async function main(){
  ...
  try{
    ...
    for(const row of scraped){const tested=[];for(const s of row.streams){const ok=await probeM3U8(s.url); if(ok) tested.push(s);} row.streams=tested;}
    const filtered=scraped.filter(r=>r.streams.length>0);
    console.log(`Channels with at least one WORKING .m3u8: ${filtered.length}`);

    const {nameMap,entries}=await parseEpgStream();

    const records=[], matchedOnly=[];
    for(const r of filtered){
      const {entry,method}=findMatch(r.channelName,r.channelNameKey,nameMap,entries);
      for(const s of r.streams){
        const rec={stream_url:s.url,channel_guess:r.channelName,epg_channel_id:entry?entry.id:null,epg_display_name:entry?(entry.names[0]||null):null,working:true,checked_at:new Date().toISOString()};
        records.push(rec);
