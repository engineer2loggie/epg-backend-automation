...
// Stream-parse multiple EPG XML.GZ files safely.
// For EPG files whose URL contains "_MX", treat all channels as Mexico.
// Otherwise, include channels whose id or any display-name has "mexico" or "mx".
// Match channels and insert into Supabase public.mx_channels:
...
async function parseOneEpg(url,agg){
  ...
  const forceAllMex = /_MX/i.test(url); // MX pack => keep all channels
  ...
  try{
    ...
    for(const r of filtered){
      ...
      for(const s of r.streams){
        const rec={
          stream_url:s.url,
          channel_guess:r.channelName,              // <-- your table column
