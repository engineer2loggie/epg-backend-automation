...
def _dedupe_rows_by_pk(rows: List[dict]) -> List[dict]:
    """
    Remove duplicates within the SAME batch by PK: (programme_source_link, programme_start_time).
    Prevents PostgREST 21000: 'cannot affect row a second time'.
    """
    seen = set()
    out = []
    for r in rows:
...
async def scrape_one(url: str, context, *, tzname: str, hours_ahead: int) -> List[Programme]:
    """
    Use a fresh Playwright page ONLY for JS-heavy sites (mvstv.com).
    GatoTV is static: let its parser fetch via httpx (pass page=None).
    """
    ...
    use_browser = netloc.endswith("mvstv.com")
